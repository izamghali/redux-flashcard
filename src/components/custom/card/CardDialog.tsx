...
export default function CardDialog({ crud, triggerBtn, triggerBtnClassName }: { crud: 'create' | 'edit' | 'delete', triggerBtn: ReactElement, triggerBtnClassName?: string }) {
    ...
    return (
        <AlertDialog open={dialogOpen} onOpenChange={setDialogOpen}>
            { crud == 'create' ?
                <AlertDialogTrigger asChild className={`${ triggerBtnClassName } flex items-center gap-2 p-2 cursor-pointer duration-200 `}>
                { triggerBtn }
                </AlertDialogTrigger>
                :
            ...
            <AlertDialogContent className="md:max-w-[40rem]">
                ...
                <div className="space-y-1 max-h-96 overflow-scroll">
                    { selectedCardType &&
                    <div className="bg-slate-400/20 p-2 rounded-md space-y-1">
                        <div className="flex items-center justify-between"><Label className="font-medium">Preview example</Label><Label className="font-light text-black/50 text-xs">Scroll down</Label></div>
                        ...
                        { selectedCardType == 'gap' && <ExampleCard cardsClassNames="h-40" frontText={"I'm really excited because I just got a ... from the office. I'll have to work longer hours, but they pay me higher now."} backText={"promotion"} /> }
                        { selectedCardType == 'picture' && <ExampleCard
                            cardsClassNames="h-52"
